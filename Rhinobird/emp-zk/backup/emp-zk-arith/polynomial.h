...
public:
	...
	void batch_check()
	{
		...
		if (party == ALICE)
		{
			...
			// coeff[0] = seed; for(int i = 1; i < sz; ++i): coeff[i] = mult_mod(coeff[i-1], seed);  即 从 seed^1 ~ seed^num
			uni_hash_coeff_gen(chi, seed, num); // chi 是 output, 即 coeff

			check_sum[0] = vector_inn_prdt_sum_red(chi, buffer, num); // check_sum = add_mod(check_sum, mult_mod(chi[i], buffer[i]));
			check_sum[1] = vector_inn_prdt_sum_red(chi, buffer1, num);
			ostriple->vole->extend(&ope_data, 1); // 生成一个VOLE关系用于mask
